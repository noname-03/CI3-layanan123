...
class Pengaduan_m extends CI_Model
{
	...
	{
		$this->db->select('pengaduan.*,masyarakat.username,masyarakat.telp');
		$this->db->from($this->table);
		$this->db->join('masyarakat', 'masyarakat.id_masyarakat = pengaduan.id_masyarakat', 'inner');
		$this->db->where('status', '0');
		return $this->db->get();
	}

	public function data_pengaduan_masyarakat_id_masyarakat($id_masyarakat)
	{
		$this->db->select('pengaduan.*,masyarakat.username,masyarakat.telp');
		$this->db->from($this->table);
		$this->db->join('masyarakat', 'masyarakat.id_masyarakat = pengaduan.id_masyarakat', 'inner');
		$this->db->where('pengaduan.id_masyarakat', $id_masyarakat);
		return $this->db->get();
	}
	...
	{
		$this->db->select('pengaduan.*, masyarakat.username, masyarakat.telp, tanggapan.tgl_tanggapan, tanggapan.tanggapan');
