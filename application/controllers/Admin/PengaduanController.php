...
class PengaduanController extends CI_Controller
{
	...
	public function index()
	{
		...
		// $data['data_pengaduan'] = $this->Pengaduan_m->data_pengaduan_masyarakat_id_masyarakat($masyarakat['id_masyarakat'])->result_array();

		$this->form_validation->set_rules('id_masyarakat', 'id_masyarakat', 'trim|required');
		...
		if (!empty($cek_data)) :
			...
			if ($cek_data['status'] == '0') :
				...
				if ($resp) :
					...
					redirect('Admin/PengaduanControllerr');
				else :
					$this->session->set_flashdata('msg', '<div class="alert alert-danger" role="alert">
